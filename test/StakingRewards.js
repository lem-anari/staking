...
describe("Contract Version 1 test", function () {
  ...
  it("I want to connecting other user and staked from it, but it `You already participated`", async () => {
    ...
    owner_balance = await token.balanceOf(user2.address);
    console.log(owner_balance);
    let tx = await token
      .connect(user2)
      .approve(stackingToken.address, stake_amount);
    await tx.wait();
    // tx = ;
    // await tx.wait();
    console.log(await token.balanceOf(user2.address));
    console.log(await stackingToken.addressStaked(user2.address));
    console.log(user1.address);
    console.log(user2.address);
    console.log(owner.address);
    await expect(
      stackingToken.connect(user2).stakeToken(stake_amount)
    ).to.changeTokenBalance(token, user2.address, -stake_amount);
  });
});
